...
// const listBalance = listAccounts.filter((item) => item.balance > 500000);
...
// tính tổng balance
const totalBalance = listAccounts.reduce((accumulator, curentValue) => 
    accumulator + curentValue.balance, 0
)
...
const statisticsByAge = listPersons.reduce(
    (accumulator, curentValue) => {
        if(curentValue.age <= 18){
            accumulator.treCon++;
        }else if(curentValue.age <= 30){
